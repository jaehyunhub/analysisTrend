...
const COMMUNITY_COLORS = ['bg-blue-500', 'bg-red-500', 'bg-orange-500', 'bg-green-500', 'bg-indigo-500', 'bg-purple-500', 'bg-pink-500', 'bg-teal-500'];
...
export default function CommunityPage() {
  const { getSortedPosts, setFilter, selectedFilter, fetchPosts, isLoading, isMember, joinedCommunities, joinCommunity, incrementMember, getMemberCount, getDailyVisitors, getWeeklyVisitors, recordVisit } = useCommunityStore();
  ...
  const displayCommunities = isSearching
  ...
  const handleFilterClick = (key: SortKey) => setFilter(key);
  ...
  const handleJoinConfirm = () => {
    ...
    incrementMember(joinConfirmTarget);
        ...
        <div className="flex w-full max-w-[1200px] gap-6">
          ...
          <div className="flex-1 py-5 flex gap-5">
            ...
            <div className="flex-1 max-w-[640px]">
              ...
              {isSearching && displayCommunities.length > 0 && (
                <div className="mb-4 bg-white dark:bg-[#1A1A1B] rounded-xl border border-gray-200 dark:border-[#343536] overflow-hidden">
                  <div className="px-4 py-3 border-b border-gray-100 dark:border-[#343536]">
                    <h3 className="text-sm font-bold text-gray-700 dark:text-gray-300">커뮤니티 검색 결과</h3>
                  </div>
                  <ul className="divide-y divide-gray-100 dark:divide-[#343536]">
                    {displayCommunities.map((comm, idx) => (
                      <li key={comm.id} className="flex items-center justify-between px-4 py-3 hover:bg-gray-50 dark:hover:bg-[#272729] transition-colors">
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-full ${COMMUNITY_COLORS[idx % COMMUNITY_COLORS.length]} flex items-center justify-center text-white text-xs font-bold shrink-0`}>
                            {comm.name[0]}
                          </div>
                          <div className="flex flex-col">
                            <Link href={`/community/board/${comm.name}`} className="font-bold text-sm text-gray-800 dark:text-gray-200 hover:underline">{comm.name}</Link>
                            <span className="text-xs text-gray-400">{getMemberCount(comm.name)}명</span>
                          </div>
                        </div>
                        <button
                          onClick={() => !isMember(comm.name) && handleJoinClick(comm.name)}
                          disabled={isMember(comm.name)}
              ...
              ) : isSearching && displayPosts.length === 0 && displayCommunities.length === 0 ? (
                <div className="text-center py-14 bg-white dark:bg-[#1A1A1B] rounded-xl border border-gray-200 dark:border-[#343536]">
            ...
            <div className="hidden lg:block w-[300px] shrink-0">
              <div className="bg-white dark:bg-[#1A1A1B] rounded-xl border border-gray-200 dark:border-[#343536] sticky top-[65px] overflow-hidden">
                <div className="h-[72px] bg-gradient-to-r from-blue-600 to-purple-600 relative flex items-end p-4">
                  <h2 className="text-white font-bold text-base drop-shadow">커뮤니티</h2>
                </div>
                <div className="p-4">
                  {(() => {
                    // 가입된 커뮤니티만 필터링 후 멤버 많은 순 정렬
                    const joinedList = communities
                      .filter((c) => joinedCommunities.includes(c.name) && !FIXED_CATEGORIES.includes(c.name))
                      .sort((a, b) => getMemberCount(b.name) - getMemberCount(a.name));

                    if (joinedList.length === 0) {
                      return (
                        <p className="text-xs text-gray-400 text-center py-4">가입된 커뮤니티가 없습니다.</p>
                      );
                    }

                    return (
                      <ul className="space-y-3">
                        {joinedList.slice(0, 5).map((comm, idx) => (
                          <li key={comm.id} className="flex items-center justify-between">
                            <div className="flex items-center gap-3 overflow-hidden">
                              <span className="text-sm font-bold text-gray-400 w-4 text-center shrink-0">{idx + 1}</span>
                              <div className={`w-7 h-7 rounded-full ${COMMUNITY_COLORS[idx % COMMUNITY_COLORS.length]} flex items-center justify-center text-white text-xs font-bold shrink-0`}>
                                {comm.name[0]}
                              </div>
                              <div className="flex flex-col min-w-0">
                                <Link
                                  href={`/community/board/${comm.name}`}
                                  className="font-bold text-sm hover:underline truncate text-gray-800 dark:text-gray-200"
                                  onClick={() => recordVisit(comm.name)}
                                >
                                  {comm.name}
                                </Link>
                                <div className="flex items-center gap-2 text-xs text-gray-400">
                                  <span>멤버 {getMemberCount(comm.name)}명</span>
                                  <span>·</span>
                                  <span>오늘 {getDailyVisitors(comm.name)}명</span>
                                </div>
                              </div>
                            </div>
                            <span className="px-3 py-1 rounded-full text-xs font-bold border border-blue-500 text-blue-500 shrink-0">가입됨</span>
                          </li>
                        ))}
                      </ul>
                    );
                  ...
                  {joinedCommunities.length > 0 && (
                    <div className="mt-4 pt-4 border-t border-gray-100 dark:border-[#343536] space-y-2">
                      <p className="text-xs font-bold text-gray-500 dark:text-gray-400">이번 주 방문자</p>
                      {communities
                        .filter((c) => joinedCommunities.includes(c.name) && !FIXED_CATEGORIES.includes(c.name))
                        .slice(0, 3)
                        .map((comm) => (
                          <div key={comm.id} className="flex items-center justify-between text-xs">
                            <span className="text-gray-700 dark:text-gray-300 truncate max-w-[150px]">{comm.name}</span>
                            <span className="font-bold text-blue-600 dark:text-blue-400">{getWeeklyVisitors(comm.name)}명</span>
                          </div>
                        ))}
                    </div>
                  )}
                  ...
                  <Link
                    href="/community"
                    className="block w-full mt-5 py-2 bg-gray-50 dark:bg-[#272729] rounded-lg text-sm font-bold text-blue-600 hover:bg-gray-100 dark:hover:bg-[#343536] transition-colors text-center"
                  >
                    전체 커뮤니티 보기
                  </Link>
                </div>
