    ...
    <>
      ...
      <main className="min-h-screen bg-gray-50 dark:bg-[#1A1A1B]">
        <div className="max-w-6xl mx-auto px-4 py-10">
              ...
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors ${
                  activeCategory === cat
                    ...
                    : 'bg-white dark:bg-[#272729] border-gray-200 dark:border-[#343536] text-gray-700 dark:text-gray-300 hover:border-blue-500 hover:text-blue-600 dark:hover:text-blue-400'
                }`}
              >
                {cat}
              </button>
          ...
          {/* 피처드 아티클 */}
          {filtered[0] && (
            <Link href={`/magazine/${filtered[0].id}`} className="block mb-8 group">
              <div className="relative h-64 md:h-80 rounded-2xl overflow-hidden">
                ...
                <div className="absolute bottom-0 left-0 p-6 text-white">
                  ...
                  <p className="text-sm text-white/80">
                    {filtered[0].author} · {filtered[0].readTime} 읽기
                  </p>
                </div>
              </div>
            </Link>
