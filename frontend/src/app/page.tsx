...
import { SCHEDULES, YOUTUBE, PRODUCTS, POSTS, BANNERS } from "@/shared/api/endpoints";
...
interface ScheduleItem {
  id: number;
  title: string;
  scheduleDate: string; // "2026-03-14"
  type: string;
  description: string;
}
...
export default function Home() {
  ...
  const broadcastNews: BroadcastNews[] = schedules.map(s => ({
    ...
    date: s.scheduleDate.slice(5).replace('-', '.'), // "03.14"
    category: s.type || '방송',
  }));
  ...
  useEffect(() => {
    // 배너
    apiGet<BannerItem[]>(BANNERS.LIST)
      .then(data => {
        const active = data
          .filter(b => b.active)
    ...
    // YouTube 영상
    apiGet<YoutubeItem[]>(YOUTUBE.LIST)
      .then(setVideos)
      .catch(() => {});

    // 상품 (4개)
    apiGet<PageResponse<ProductItem>>(`${PRODUCTS.LIST}?page=0&size=4`)
