...
| ID | 기능 | 우선순위 | 현황 |
|----|------|---------|------|
| HOME-01 | Hero 배너 (관리자 CRUD 데이터 연동) | P0 | UI 있음, 하드코딩 |
| HOME-02 | 최신 YouTube 영상 그리드 (YouTube Data API v3 연동) | P0 | UI 있음, 하드코딩 |
| HOME-03 | 주간 방송 일정 위젯 (관리자 일정 데이터 연동) | P1 | UI 있음, 하드코딩 |
...
| ID | 기능 | 우선순위 | 현황 |
|----|------|---------|------|
| COMM-01 | 게시물 CRUD (Economy / Broadcast / Shopping / Free 카테고리) | P0 | UI만, API 없음 |
| COMM-02 | 업보트/다운보트 (Optimistic Update, 중복 투표 취소) | P0 | UI만, 미동작 |
| COMM-03 | 게시물 정렬 (Hot · Best · New · Top 알고리즘) | P0 | UI만, 미동작 |
| COMM-04 | 댓글 및 대댓글 작성/삭제 | P1 | UI만, 미동작 |
| COMM-05 | 게시물 검색 (제목·본문, 디바운스) | P1 | 컴포넌트 존재 |
...
| ID | 기능 | 우선순위 | 현황 |
|----|------|---------|------|
| ADM-01 | 홈 배너 CRUD (이미지, 링크, 순서) | P0 | UI만, API 미연동 |
| ADM-02 | 방송 일정 CRUD (날짜, 시간, 제목, 주제) | P0 | UI만, API 미연동 |
| ADM-03 | YouTube 영상 링크 관리 (URL, 썸네일, 설명) | P0 | UI만, API 미연동 |
| ADM-04 | 광고 슬롯 관리 | P1 | UI만, API 미연동 |
| ADM-05 | 커뮤니티 게시물 관리 (신고 처리, 강제 삭제) | P1 | UI만, API 미연동 |
| ADM-06 | 회원 관리 (역할 변경, 계정 정지) | P1 | UI만, API 미연동 |
...
| ID | 기능 | 우선순위 | 현황 |
|----|------|---------|------|
| ANA-01 | 채널 KPI 카드 (구독자 수·조회수·시청 시간·평균 시청 시간) | P0 | UI Mock 존재 |
| ANA-02 | 구독자 증감 추이 차트 (30/90/365일 선택) | P0 | CSS Mock 차트 존재 |
| ANA-03 | 상위 퍼포밍 영상 목록 (조회수·CTR·시청 시간 기준) | P0 | UI Mock 존재 |
...
| ID | 기능 | 우선순위 | 현황 |
|----|------|---------|------|
| TRD-01 | YouTube 실시간 급상승 동영상 (국가·카테고리 필터) | P0 | UI Mock 존재 |
| TRD-02 | 실시간 뉴스 키워드 클라우드 (30분 주기 갱신) | P0 | UI Mock 존재 |
...
| ID | 기능 | 우선순위 | 현황 |
|----|------|---------|------|
| CHAT-01 | 라이브 채팅 로그 파일 업로드 (CSV / JSON / TXT) | P1 | 미구현 |
| CHAT-02 | 특정 키워드 반복 시간대 목록 추출 | P1 | 미구현 |
| CHAT-03 | 채팅 밀도 타임라인 히트맵 시각화 | P1 | 미구현 |
| CHAT-04 | 피크 구간 자동 추출 및 편집 포인트 추천 | P1 | 미구현 |
| CHAT-05 | 편집 마커 CSV 내보내기 | P1 | 미구현 |
